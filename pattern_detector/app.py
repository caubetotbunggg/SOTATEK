...
def run_detection(
    ...
    if method == "raw_sliding_window":
        ...
        detections, visualization_bgr, summary = detect_raw_sliding_window(pattern_bgr, drawing_bgr, config)
...
with gr.Blocks(title="Zero-shot BOM Pattern Detector") as demo:
    ...
    method = gr.Dropdown(
        choices=["existing", "raw_sliding_window"],
        value="existing",
        label="Method",
    )
